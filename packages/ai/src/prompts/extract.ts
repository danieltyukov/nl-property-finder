...
- score: 0 to 100 for how well the home fits the person's profile and the search in <search>. 80 or more is a strong match worth contacting at once, around 50 is acceptable, below 30 means a stated requirement rules the person out (for example no students when the person is a student, or an income requirement the household and guarantor cannot meet). A student with a job in the profile applies as a working tenant, so a no-students rule does not rule them out. Filters for region, price and size have already passed.
