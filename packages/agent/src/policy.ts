import type {
  Application, ApplicationStatus, AutomationConfig, ClassifyOutput, Intent, ReplyInput, ScamVerdict, TaskKind,
} from '@nlpf/core';
...
const task = (t: TaskKind, priority: 1 | 2 | 3, reason: string): TaskAction => ({ kind: 'task', task: t, priority, reason });
...
export function taskFor(intent: Intent): TaskAction {
  switch (intent) {
    ...
    case 'info_request':
      return task('reply_needed', 2, 'The landlord asked something the agent cannot answer from your profile.');
    ...
    case 'payment_request':
      return task('payment_warning', 1, 'The landlord asks for a payment. Do not pay before a viewing and a signed contract.');
    case 'scam_suspect':
...
const isAutomatic = (a: PolicyAction) => a.kind === 'auto_reply' || a.kind === 'book_viewing' || a.kind === 'close';
...
export function decidePolicy(intent: Intent, ctx: PolicyContext): PolicyAction {
  ...
  if (!ctx.application) return task('reply_needed', 2, 'A reply arrived that matches none of your applications.');
  if (action.kind === 'close') {
    return ctx.classification.confidence < MIN_AUTO_CONFIDENCE ? task('reply_needed', 2, 'This looks like a rejection, but the agent is not sure.') : action;
  }
  if (ctx.application.status === 'withdrawn') return task('reply_needed', 3, 'A landlord wrote about an application you withdrew.');
  if (ctx.scam.level === 'likely') return task('scam_review', 2, 'This conversation shows scam signals, so the agent did not reply.');
  if (ctx.classification.confidence < MIN_AUTO_CONFIDENCE) return taskFor(intent === 'info_request' ? 'other' : intent);
