...
const MAX_RENT_2026 = [
  250.26, 256.53, 262.75, 269.02, 275.27, 281.5, 287.78, 294.03, 300.29, 306.54,
  312.8, 319.02, 325.3, 331.54, 337.8, 344.05, 350.35, 356.53, 362.79, 369.09,
  375.32, 381.55, 387.83, 394.06, 400.32, 406.58, 412.85, 419.1, 425.33, 431.56,
  437.81, 444.09, 450.36, 456.57, 462.86, 469.09, 475.36, 481.6, 487.89, 494.1,
  500.38, 507.22, 514.08, 520.96, 527.81, 534.7, 541.56, 548.41, 555.29, 562.13,
  569.03, 575.87, 582.71, 589.61, 596.45, 603.32, 610.19, 617.08, 623.94, 630.82,
  637.67, 644.53, 651.36, 658.24, 665.12, 671.95, 678.85, 685.7, 692.56, 699.44,
  706.32, 713.2, 720.05, 726.9, 733.79, 740.66, 747.51, 754.37, 761.21, 768.08,
  774.94, 781.85, 788.71, 795.56, 802.44, 809.3, 816.14, 823.02, 829.94, 836.74,
  843.62, 850.49, 857.33, 864.24, 871.06, 877.97, 884.79, 891.67, 898.56, 905.39,
  912.26, 919.14, 925.98, 932.93, 939.73, 946.61, 953.45, 960.33, 967.18, 974.05,
  980.91, 987.78, 994.63, 1001.5, 1008.35, 1015.22, 1022.07, 1029, 1035.81, 1042.73,
  1049.57, 1056.42, 1063.32, 1070.14, 1077, 1083.88, 1090.76, 1097.61, 1104.46, 1111.39,
  1118.23, 1125.08, 1131.94, 1138.85, 1145.69, 1152.55, 1159.4, 1166.27, 1173.15, 1180.01,
  1186.84, 1193.76, 1200.61, 1207.46, 1214.31, 1221.21, 1228.07, 1234.92, 1241.81, 1248.65,
  1255.53, 1262.4, 1269.25, 1276.12, 1283, 1289.86, 1296.7, 1303.57, 1310.46, 1317.28,
  1324.18, 1331.03, 1337.89, 1344.75, 1351.63, 1358.5, 1365.34, 1372.24, 1379.09, 1385.95,
  1392.84, 1399.69, 1406.56, 1413.43, 1420.28, 1427.15, 1433.99, 1440.86, 1447.71, 1454.6,
  1461.49, 1468.31, 1475.19, 1482.05, 1488.95, 1495.77, 1502.67, 1509.53, 1516.4, 1523.28,
  1530.12, 1536.98, 1543.85, 1550.71, 1557.56, 1564.46, 1571.31, 1578.17, 1585.01, 1591.91,
  1598.76, 1605.64, 1612.52, 1619.36, 1626.24, 1633.1, 1639.96, 1646.78, 1653.7, 1660.54,
  1667.4,
];
...
const LABEL_POINTS: Record<string, [number, number]> = {
  'A++++': [62, 58], 'A+++': [57, 53], 'A++': [52, 48], 'A+': [47, 43], A: [41, 37],
  B: [34, 30], C: [22, 15], D: [14, 11], E: [-4, -4], F: [-9, -9], G: [-15, -15],
};
 ...
 */
export function energyPoints(label: string | undefined, type: PropertyType | undefined, buildYear?: number): number {
...
export interface MaxRentEstimate { points: number; maxRentEur: number; sector: RentCheck['sector']; note: string }
...
export function estimateMaxRent(input: MaxRentInput): MaxRentEstimate {
  ...
  const rooms = input.type === 'studio' || input.type === 'room' ? 1 : input.rooms ?? Math.max(1, Math.round(m2 / 25));

  const area = m2;
  const heating = 2 * rooms + TYPICAL.heatedOtherSpaces;
  const energy = energyPoints(label, input.type, label ? undefined : input.labelChecked ? input.buildYear : undefined);
  ...
  const sector: RentCheck['sector'] = points <= SOCIAL_MAX_POINTS ? 'social' : points <= MIDDLE_MAX_POINTS ? 'middle' : 'free';
  ...
  const parts = [
    `Estimate under the 2026 point system (WWS): about ${points} points, so a maximum base rent of about ${eur(maxRentEur)}`
      + (sector === 'free' ? ', which puts it in the free sector where no legal maximum applies.' : ` (${sector === 'social' ? 'social' : 'regulated middle'} rent).`),
  ];
  if (!label) parts.push(input.labelChecked && input.buildYear ? `No valid energy label, so the build year ${input.buildYear} sets the energy points.` : 'Energy label unknown, label A assumed.');
  if (input.wozEur === undefined) parts.push(`WOZ value unknown, the legal minimum of ${eur(MINIMUM_WOZ_2026)} was used, so the real maximum is probably higher.`);
  if (capped) parts.push('The WOZ share was capped at 33%.');
  parts.push('Kitchen, bathroom, heating and outdoor space are typical assumptions. Only the Huurcommissie can set the real maximum.');
 ...
 */
export function rentCheck(listing: Listing, facts: Omit<PropertyFacts, 'sources'> & { sources: RentCheck['sources'] }, now: Date): RentCheck | undefined {
  ...
  if (fromListing(facts.sizeM2, listing.sizeM2) || fromListing(facts.energyLabel, listing.energyLabel)) sources.push('listing');
  ...
  const est = estimateMaxRent({
    sizeM2, energyLabel, type: listing.type, wozEur: facts.wozEur, wozPeildatum: facts.wozPeildatum,
    buildYear: facts.buildYear, labelChecked: facts.labelChecked, rooms: listing.rooms,
  });
  ...
  const note = now.getUTCFullYear() > TABLE_YEAR ? `${est.note} The ${TABLE_YEAR} tables were used; newer ones may apply.` : est.note;
  const out: RentCheck = { points: est.points, maxRentEur: est.maxRentEur, sector: est.sector, inputs, sources, note };
  const base = listing.priceEur === undefined ? undefined : listing.priceBasis === 'incl' ? listing.priceEur - (listing.serviceCostsEur ?? 0) : listing.priceEur;
