...
const near = listing({ address: { city: 'Delft', lat: 52.0067, lon: 4.3556 }, description: 'Licht appartement met balkon en berging.' });
const far = listing({ address: { city: 'Rotterdam', lat: 51.9244, lon: 4.469 }, description: 'Appartement zonder buitenruimte.' });
...
test('commute without a limit is only a reason, and unknown coordinates are skipped', () => {
  ...
  expect(applyPreferences({ score: 70, reasons: [] }, near, search({ commute: [info] }))).toEqual({ score: 70, reasons: ['8 min by bike to TU Delft'] });
  expect(applyPreferences({ score: 70, reasons: [] }, listing(), search({ commute: [tu] }))).toEqual({ score: 70, reasons: [] });
});

test('the score stays between 0 and 100', () => {
  expect(applyPreferences({ score: 98, reasons: [] }, near, search({ mustHaves: ['balkon', 'berging'] })).score).toBe(100);
  expect(applyPreferences({ score: 5, reasons: [] }, far, search({ commute: [tu], mustHaves: ['tuin'] })).score).toBe(0);
});
