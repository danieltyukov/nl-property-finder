...
const pdok = (q: string) => `https://api.pdok.nl/bzk/locatieserver/search/v3_1/free?q=${encodeURIComponent(q)}&fq=type:adres&rows=1`;

test('builds the Locatieserver URL from the address', () => {
  expect(pdokQueryUrl({ street: 'Oude Delft', houseNumber: '12-a', city: 'Delft' })).toBe(pdok('Oude Delft 12A Delft'));
  expect(pdokQueryUrl({ street: 'Oude Delft', houseNumber: '12', postcode: '2611 bc', city: 'Delft' })).toBe(pdok('Oude Delft 12 2611BC Delft'));
...
test('fills postcode, municipality, neighbourhood and coordinates from PDOK', async () => {
  ...
  expect(out).toMatchObject({
    street: 'Oude Delft', houseNumber: '12', addition: 'A', postcode: '2611 CC', city: 'Delft', municipality: 'Delft', neighbourhood: 'Centrum',
  });
...
test('answers from the store cache the second time', async () => {
  ...
  const again = await createGeocoder(store, fetchJson).geocode({ street: 'Oude Delft', houseNumber: '12', addition: 'a', city: 'Delft' });
...
test('a wrong postcode is retried without it and corrected', async () => {
  const fetchJson = fixtureFetch();
  const out = await createGeocoder(openStore(':memory:'), fetchJson).geocode({ street: 'Oude Delft', houseNumber: '12', postcode: '2611 BC', city: 'Delft' });
  ...
  expect(fetchJson.calls.map((c) => c.url)).toEqual([pdok('Oude Delft 12 2611BC Delft'), pdok('Oude Delft 12 Delft')]);
});
...
test('an address without a house number is not looked up', async () => {
  const fetchJson = fixtureFetch();
  expect(await createGeocoder(openStore(':memory:'), fetchJson).geocode({ city: 'Delft' })).toEqual({ city: 'Delft' });
