...
const mail = (over: Partial<InboundMessage> = {}): InboundMessage => ({
  id: '<a@b>', channel: 'email', from: { address: 'verhuur@delftrentals.nl' }, subject: 'Re: Oude Delft 12A', text: 'Dank voor uw reactie.', at: 't', attachments: [], ...over,
});
...
test('after 3 replies in a day replyBudgetLeft is 0', () => {
  ...
  const c = store.conversations.create({ applicationId: null, propertyId: null, counterpart: { email: 'x@y.nl' }, lastMessageAt: 't', unread: 0 });
  const out = (at: string, author: Message['author'] = 'agent') =>
    store.messages.add({ conversationId: c.id, direction: 'out', author, channel: 'email', body: 'x', at, status: 'sent' });
