...
const dutch = listing({
  description: 'Mooi appartement in het centrum van Delft, per direct beschikbaar voor een net persoon.',
});
const english = listing({
  description: 'Lovely apartment in the centre of Delft, available now for a tidy tenant.',
  language: 'en',
});
...
describe('messageLanguage', () => {
  test('auto follows the listing, an explicit choice wins', () => {
    ...
    expect(messageLanguage(profile(), listing({ description: undefined, title: 'Oude Delft 12A' }))).toBe(
      'nl',
    );
  });
});
...
test('composeInput picks the template for the language and the channel limit', () => {
  ...
  expect(composeInput(dutch, profile(), a, 'form', 'short')).toMatchObject({
    template: 'Beste {firstName}',
    language: 'nl',
    channel: 'form',
    maxChars: 2000,
    variant: 'short',
  });
  expect(composeInput(english, profile(), a, 'message')).toMatchObject({
    template: 'Dear landlord',
    language: 'en',
    maxChars: 1500,
  });
...
describe('finaliseMessage', () => {
  test('removes dashes used as punctuation and emojis', () => {
    ...
    expect(cleanCopy('Een twee-onder-een-kapwoning, 2026-11-01')).toBe(
      'Een twee-onder-een-kapwoning, 2026-11-01',
    );
  });
  ...
  test('adds the disclosure line once, in the message language', () => {
    const a = automation({
      disclosure: {
        enabled: true,
        nl: 'Dit bericht is opgesteld met hulp van mijn assistent.',
        en: 'This message was drafted with help from my assistant.',
      },
    });
...
test('built-in withdrawal and follow-up texts', () => {
  ...
  expect(withdrawalText(automation({ withdraw: { nl: 'Eigen tekst, {firstName}', en: '' } }), p, 'nl')).toBe(
    'Eigen tekst, Sam',
  );
