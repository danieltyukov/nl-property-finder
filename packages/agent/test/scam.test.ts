...
test('an ordinary listing has no signals', () => {
  const l = listing({
    description: 'Lichte woning met balkon. Bezichtiging op afspraak via ons kantoor.',
    priceEur: 1250,
    sizeM2: 40,
  });
...
test('Dutch phrasings', () => {
  const l = listing({
    description:
      'Ik woon momenteel in het buitenland. Maak eerst de borg over, dan stuur ik de sleutels per post. Alleen contact via WhatsApp: +44 7700 900123.',
  });
  expect(scamSignals(l, {})).toEqual(
    expect.arrayContaining(['landlord_abroad', 'payment_before_viewing', 'keys_by_post', 'whatsapp_only']),
  );
});
...
test('off-platform contact and a missing address', () => {
  const l = listing({
    address: { city: 'Delft' },
    description: 'Please email me directly at verhuur.delft1970@gmail.com for details.',
  });
...
test('too good to be true stories', () => {
  const l = listing({
    description:
      "God bless you. I am an honest man and I only want a trustworthy tenant for my late mother's apartment, no questions asked.",
  });
  expect(scamSignals(l, {})).toContain('too_good_description');
});

test('a price below median only counts with a median and a size', () => {
  expect(scamSignals(listing({ priceEur: 300, sizeM2: undefined }), { medianPricePerM2: 30 })).not.toContain(
    'price_far_below_median',
  );
...
test('an agent office address on the listing is not off-platform contact', () => {
  const l = listing({
    agent: { name: 'Makelaardij Delft', email: 'info@makelaardijdelft.nl' },
    description: 'Neem contact op met info@makelaardijdelft.nl.',
  });
  expect(scamSignals(l, {})).not.toContain('off_platform_contact');
});

test('verdict bundles level and signals', () => {
  expect(scamVerdict(['payment_before_viewing'])).toEqual({
    level: 'possible',
    signals: ['payment_before_viewing'],
  });
