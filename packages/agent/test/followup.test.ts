...
const app: Application = {
  ...
  propertyId: 'p_1',
  status: 'contacted',
  firstSeenAt: '2026-09-20T09:59:00Z',
...
const msg = (direction: 'in' | 'out', at: string): Message => ({
  ...
  conversationId: 'c_1',
  direction,
  author: direction === 'in' ? 'landlord' : 'agent',
  channel: 'email',
  body: 'x',
  at,
...
test('not due once the landlord replied', () => {
  expect(
    followUpDue(app, msg('in', '2026-09-21T08:00:00Z'), first, cfg, new Date('2026-09-25T10:00:00Z')),
  ).toBe(false);
});

test('an inbound message from before the contact does not count as a reply', () => {
  expect(
    followUpDue(app, msg('in', '2026-09-19T08:00:00Z'), first, cfg, new Date('2026-09-25T10:00:00Z')),
  ).toBe(true);
});
...
test('not due when the application moved on or closed', () => {
  for (const status of ['replied', 'viewing_booked', 'rejected', 'withdrawn', 'gone', 'offer'] as const) {
    expect(followUpDue({ ...app, status }, undefined, first, cfg, new Date('2026-09-30T10:00:00Z'))).toBe(
      false,
    );
  }
});
...
test('the number of follow-ups is capped', () => {
  ...
  expect(followUpDue(app, undefined, followUp, { ...cfg, max: 2 }, new Date('2026-09-25T10:00:00Z'), 1)).toBe(
    false,
  ); // counts from the last message
  expect(followUpDue(app, undefined, first, { ...cfg, max: 0 }, now)).toBe(false);
});

test('switched off means never', () => {
  expect(
    followUpDue(app, undefined, first, { ...cfg, enabled: false }, new Date('2026-10-30T10:00:00Z')),
  ).toBe(false);
});
