...
describe('the official numbers', () => {
  ...
  test('WOZ points follow the Huurcommissie example', () => {
    ...
    expect(wozPoints(300_000, 60, '2024-01-01')).toBe(Math.round((300_000 / 15_329 + 300_000 / 60 / 242) * 4) / 4);
  });
...
describe('estimateMaxRent', () => {
  ...
  test('sectors: social up to 143 points, middle 144 to 186, free from 187', () => {
    expect(estimateMaxRent({ sizeM2: 60, energyLabel: 'A', type: 'apartment', wozEur: 300_000 }).sector).toBe('middle');
    expect(estimateMaxRent({ sizeM2: 110, energyLabel: 'A+', type: 'house', wozEur: 650_000 }).sector).toBe('free');
  });
  ...
  test('without a WOZ value the legal minimum is used and the note says so', () => {
    ...
    expect(r.points).toBeLessThan(estimateMaxRent({ sizeM2: 30, energyLabel: 'A', type: 'studio', wozEur: 180_000 }).points);
  });
});

describe('rentCheck', () => {
  const facts = { sizeM2: 30, energyLabel: 'A', wozEur: 180_000, buildYear: 2004, sources: ['bag', 'woz', 'ep-online'] as ('bag' | 'woz' | 'ep-online')[] };
  ...
  test('a studio asking 40% more than the estimate is about 40% above', () => {
    ...
    expect(r).toMatchObject({ points: 119, maxRentEur: 768.08, sector: 'social', sources: ['bag', 'woz', 'ep-online'] });
  ...
  test('service costs are left out of an inclusive price', () => {
    const l = listing({ type: 'studio', priceEur: 900, priceBasis: 'incl', serviceCostsEur: 131.92, sizeM2: 30 });
    expect(rentCheck(l, facts, now)?.aboveMaxPct).toBeCloseTo(0, 0);
  });

  test('listing data fills gaps and is named as a source', () => {
    const r = rentCheck(listing({ type: 'studio', priceEur: 700, sizeM2: 30, energyLabel: 'A' }), { wozEur: 180_000, sources: ['woz'] }, now);
  ...
  test('no comparison without an official WOZ value or in the free sector', () => {
    expect(rentCheck(listing({ type: 'studio', priceEur: 1200, sizeM2: 30 }), { sources: [] }, now)?.aboveMaxPct).toBeUndefined();
    const big = rentCheck(listing({ type: 'house', priceEur: 3000, sizeM2: 140 }), { sizeM2: 140, energyLabel: 'A', wozEur: 800_000, sources: ['bag', 'woz'] }, now);
  ...
  test('an out-of-date table is mentioned', () => {
    const r = rentCheck(listing({ type: 'studio', priceEur: 900, sizeM2: 30 }), { wozEur: 180_000, sources: ['woz'] }, new Date('2027-02-01T00:00:00Z'));
    expect(r?.note).toMatch(/2026 tables were used/);
    expect(rentCheck(listing({ type: 'studio', priceEur: 900, sizeM2: 30 }), { wozEur: 180_000, sources: ['woz'] }, now)?.note).not.toMatch(/tables were used/);
  });
