...
const slot = (start: string, over: Partial<ProposedSlot> = {}): ProposedSlot => ({
  start,
  text: start,
  certain: true,
  ...over,
...
test('skips slots outside availability and uncertain slots', () => {
  expect(
    chooseSlot([thu2100, slot('2026-09-25T07:00:00.000Z', { certain: false })], availability, 45, []),
  ).toBeNull();
});
...
test('a slot must end inside availability', () => {
  ...
  expect(
    chooseSlot([slot('2026-09-24T17:45:00.000Z', { end: '2026-09-24T17:55:00.000Z' })], availability, 0, []),
  ).not.toBeNull();
});
...
test('an open window is narrowed to the first free half hour', () => {
  const window = slot('2026-09-25T15:00:00.000Z', {
    end: '2026-09-25T17:00:00.000Z',
    text: 'vrijdag tussen 17 en 19 uur',
  }); // 17:00-19:00 local
  ...
  expect(chooseSlot([window], availability, 45, busy)).toEqual({
    ...window,
    start: '2026-09-25T16:15:00.000Z',
    end: '2026-09-25T16:45:00.000Z',
  });
});
