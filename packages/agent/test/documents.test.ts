...
const files: DocumentFile[] = [
  ...
  {
    name: 'Werkgeversverklaring.pdf',
    path: '/d/employer.pdf',
    sensitivity: 'private',
    kind: 'employer_statement',
  },
...
describe('documentsToSend', () => {
  test('identity documents always need approval', () => {
    ...
    const permissive = {
      public: 'auto',
      private: 'after_viewing_booked',
      identity: 'auto',
    } as unknown as typeof policy;
    expect(
      names(documentsToSend(['passport'], permissive, { viewingBooked: true, scam: none, files }).approve),
    ).toEqual(['id']);
  });
  ...
  test('private documents go out only after a viewing is booked and with no scam signals', () => {
    ...
    expect(
      names(documentsToSend(requested, policy, { viewingBooked: true, scam: none, files }).send),
    ).toEqual(['payslip', 'employer_statement']);
    expect(
      names(documentsToSend(requested, policy, { viewingBooked: false, scam: none, files }).approve),
    ).toEqual(['payslip', 'employer_statement']);
    expect(
      names(documentsToSend(requested, policy, { viewingBooked: true, scam: possible, files }).approve),
    ).toEqual(['payslip', 'employer_statement']);
  ...
  test('public documents go out automatically unless a scam is likely or the policy says approve', () => {
    expect(
      names(documentsToSend(['huurdersprofiel'], policy, { viewingBooked: false, scam: none, files }).send),
    ).toEqual(['tenant_profile']);
    expect(
      names(
        documentsToSend(['profile'], policy, {
          viewingBooked: false,
          scam: { level: 'likely', signals: [] },
          files,
        }).approve,
      ),
    ).toEqual(['tenant_profile']);
    expect(
      names(
        documentsToSend(
          ['profile'],
          { ...policy, public: 'approve' },
          { viewingBooked: false, scam: none, files },
        ).approve,
      ),
    ).toEqual(['tenant_profile']);
  });
  ...
  test('a general request sends the public documents only', () => {
    expect(documentsToSend(['documenten'], policy, { viewingBooked: true, scam: none, files })).toEqual({
      send: [files[0]],
      approve: [],
    });
    expect(documentsToSend([], policy, { viewingBooked: true, scam: none, files })).toEqual({
      send: [files[0]],
      approve: [],
    });
  });
  ...
  test('English names and missing documents', () => {
    const out = documentsToSend(['payslips', 'proof of enrolment'], policy, {
      viewingBooked: true,
      scam: none,
      files,
    });
